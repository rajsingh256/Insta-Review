...
$movies = readMovies('data/romance.txt');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
foreach ($movies as $mov) {
  ...
  echo '<td class="mdl-data-table__cell--non-numeric"><span style="font-size: 30px;"><a href="romance.php?movie=' . $mov['name'] . '">' . $mov['name'] . '</a></td>';
...
echo '<h4>' . '<b>' . $requestedMovie['name'] .'</h4>'; 
...
<div class="mdl-card__supporting-text">
<?php
    $reviews = readReviews($_GET['movie'], 'data/romance.txt');
    if (is_null($reviews)) {
